				...
				(await Countdown.all()).forEach(async countdown => {
					try {
						...
						if (timeDiff > 0) {
							...
							countdown.unixTime += 24*60*60;
							countdown.description = countdown.description.slice(0, countdown.description.length-1) + (countdown.description.slice(countdown.description.length-1) -(-1));
						}
					} catch (err) {
