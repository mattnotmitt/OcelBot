...
exports.func = async (msg, args, bot) => {
	try {
		...
		let emoteList;
		if (guildEmotes.length <= 50) {
			emoteList = [new Discord.RichEmbed({
		...
		try {
			...
			if (guildEmotes.length <= 50) {
				Promise.all(emoteList.map(emoteEmbed => dm.send('', emoteEmbed)));
			}
			msg.reply(`The available emotes on this server can be viewed at https://ocel.artemisbot.uk/emotes/list/${guildId}\n ${guildEmotes.length <= 50 ? 'I have also DMed you with a list of these emotes.' : ''}`);
		} catch (err) {
